...
class ButtonGenerateDtm (bpy.types.Operator):
    ...
    '''
    bl_idname = 'ButtonGenerateDtm' # name used to refer to this operator
    ...
    def invoke(self, context, event):
        ...
        print ('INVOKING'+geoLoaderInfo.dtm_file_path)
...
class ButtonFileChooserDtm (bpy.types.Operator):
    ...
    Button to check the files and call the loading of the DTM files
    '''
    bl_idname = 'ButtonFileChooserDtm' # name used to refer to this operator
...
class PanelGeoDataLoader (bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not os.path.exists(geoLoaderInfo.dtm_file_path):
            selectBox.operator(operator='ButtonFileChooserDtm', text='Choose a DTM File')
        else:
            # Existing file
            selectBox.operator(operator='ButtonFileChooserDtm', text='File: '+geoLoaderInfo.dtm_file_name)
            # Adding a new box with generate buttong 
            #generateBox = selectBox.box()
            selectBox.operator(operator='ButtonGenerateDtm', text='Generate DTM')
