...
A good explanation on the use of Blender's interface can be found at: \href{http://en.wikibooks.org/wiki/Blender_3D:_Noob_to_Pro/Learning_Graphic_Interface}{http://en.wikibooks.org/wiki/Blender{{\textbackslash}textunderscore}3D:{{\textbackslash}textunderscore}Noob{{\textbackslash}textunderscore}to{{\textbackslash}textunderscore}Pro/Learning{{\textbackslash}textunderscore}Graphic{{\textbackslash}textunderscore}Interface}.
...
It is recommended to switch the Blender 3D View window to fullscreen before starting a simulation. Press \key{Ctrl + } when the mouse is over a window to toggle it to fullscreen and back.
